(function (global, factory) {
    ...
    d3__namespace.selection.prototype.moveToFront = function () {
        return this.each(function () {
            this.parentNode.appendChild(this);
        });
    };

    d3__namespace.selection.prototype.moveToBack = function () {
        return this.each(function () {
    ...
    function syncSettings(settings) {
        ...
        if (settings.hasOwnProperty('visitn_col')) settings.visit_order_col = settings.visitn_col; // Handle a string argument to filters.

        if (!(settings.filters instanceof Array))
            settings.filters = typeof settings.filters === 'string' ? [settings.filters] : []; // Handle a string argument to detail.
        ...
        if (settings.visits.hasOwnProperty('baseline'))
            settings.visits.timepoint1 = settings.visits.baseline; // Map [ visits.comparison ] to [ visits.timepoint1 ] to maintain backwards compatibility.
    ...
    function cleanData() {
        ...
        var nRemoved = nPreclean - nClean; //Warn user of removed records.

        if (nRemoved > 0)
            console.warn(
                ''
                    .concat(nRemoved, ' missing or non-numeric result')
                    .concat(nRemoved > 1 ? 's have' : ' has', ' been removed.')
    ...
    function trimMeasures() {
        ...
        this.initial_data.forEach(function (d) {
            d[_this.config.measure_col] = d[_this.config.measure_col].trim(); //Concatenate unit to measure if provided.

            d.sdd_measure = d.hasOwnProperty(_this.config.unit_col)
    ...
    function getMeasures() {
        ...
        this.measures = this.initial_data[0].hasOwnProperty(this.config.measure_order_col)
            ...
            : _toConsumableArray(
              ...
              ).sort(webcharts.dataOps.naturalSorter); // Define set of measure values with units (in ADaM units are already attached; in SDTM units are captured in a separate variable).
    ...
    function getVisits() {
        ...
        if (
            ...
            this.visits = _toConsumableArray(
                new Set(
                    this.initial_data.map(function (d) {
    ...
    function updateControlInputs() {
        ...
        var timepoint1_control = this.controls.config.inputs.find(function (input) {
            return input.option === 'visits.timepoint1';
        });
        timepoint1_control.values = this.visits;
        timepoint1_control.start = this.config.visits.timepoint1;
        var timepoint2_control = this.controls.config.inputs.find(function (input) {
            return input.option === 'visits.timepoint2';
        });
    ...
    function initSettings() {
        ...
        this.config.measure.x = this.config.measure.x || this.sdd_measures[0]; //  this.config.x.column = this.config.measure.x;

        this.config.measure.y = this.config.measure.y || this.sdd_measures[1]; //Set timepoint 1 and timepoint 2 visits.

        this.config.visits.timepoint1 =
            this.config.visits.timepoint1.length > 0
                ? this.config.visits.timepoint1
                : [this.visits[0]];
    ...
    function onInit() {
        ...
        cleanData.call(this); // 2. trim measures.

        trimMeasures.call(this); // 3a Check filters against data.

        checkFilters.call(this); // 3b Get list of measures.

        getMeasures.call(this); // 3c Get list of visits.

        getVisits.call(this); //4a. Initialize the delta-delta settings &  Update control inputs.

        initSettings.call(this);
        updateControlInputs.call(this); //initialize custom events
    ...
    function initNotes() {
        ...
        this.wrap
            ...
            .text('Click a point to see details.'); //Add header element in which to list visits at which measure is captured.

        this.wrap.append('p', 'svg').attr('class', 'possible-visits'); //Add element for participant counts.
    ...
    function updateVisitControls() {
        ...
        this.config;
        var timepoint1Select = this.controls.wrap
            .selectAll('.control-group')
            .filter(function (f) {
                return f.option === 'visits.timepoint1';
            })
            .select('select');
        timepoint1Select
            .selectAll('option')
            .filter(function (f) {
                return _this.config.visits.timepoint1.indexOf(f) > -1;
            })
            .attr('selected', 'selected');
        var timepoint2Select = this.controls.wrap
            .selectAll('.control-group')
            .filter(function (f) {
                return f.option === 'visits.timepoint2';
            })
            .select('select');
        timepoint2Select
            .selectAll('option')
            .filter(function (f) {
                return _this.config.visits.timepoint2.indexOf(f) > -1;
            })
            .attr('selected', 'selected');
    }
    ...
    function updateAxisSettings() {
        var config = this.config; //set config properties here since they aren't available in onInit
    ...
    function updateParticipantCount(chart, selector, id_unit) {
        ...
        var totalObs = d3
            ...
            .values().length; //count the number of unique ids in the current chart and calculate the percentage
        ...
        var percentage = d3.format('0.1%')(currentObs / totalObs); //clear the annotation

        var annotation = d3.select(selector);
        annotation.selectAll('*').remove(); //update the annotation
    ...
    function onDraw() {
        ...
        updateParticipantCount(this, '.annote'); //Reset things.
    ...
    function drawBoxPlot(
        ...
        var horizontal = horizontal == undefined ? true : horizontal; //make the results numeric and sort

        var results = results
            .map(function (d) {
                return +d;
            })
            .sort(d3.ascending); //set up scales
        ...
        var boxplot = svg.append('g').attr('class', 'boxplot').datum({
            values: results,
            probs: probs,
        ...
        boxplot
            ...
            .style('fill', boxColor); //draw dividing lines at median, 95% and 5%
            ...
            this.tbody
                ...
                .each(function (row_d) {
                    ...
                    var x = d3.scale
                        .ordinal()
                        .domain(chart.visits)
                        .rangeBands([offset, width - offset]); //y-domain includes 99th population percentile + any participant outliers
                    ...
                    var y = d3.scale
                        ...
                        .range([height - offset, offset]); //render the svg
                    ...
                    canvas.append('path').datum(overTime).attr({
                        class: 'sparkLine',
                        d: draw_sparkline,
                        fill: 'none',
                        stroke: '#999',
                    ...
                    canvas
                        ...
                        .append('title')
                        .text(function (d) {
                            return ''.concat(d[config.visit_col], ': ').concat(d[config.value_col]);
                        });
                });
        }
    }
    ...
    function showParticipantDetails(d) {
        var table = this;
        var chart = this.chart;
        var raw = d.raw[0]; //show detail variables in a ul
    ...
    function addRegressionLine() {
        if (this.config.add_regression_line) {
            var chart = this;
            this.config; // map chart data to array and calculate regression using regression-js
            ...
            var result = regression.linear(arrayData); //calculate predicted values for min and max points on the chart

            var min_x = chart.x_dom[0];
            var min_xy = result.predict(min_x);
            var max_x = chart.x_dom[1];
            var max_xy = result.predict(max_x); //draw the regression line
            ...
            chart.svg
                ...
                .attr('d', line)
                .attr('stroke', 'black')
                .attr('stroke-dasharray', '3,5'); //add footnote with R2 and exact calculation
    ...
    function layout(element) {
        ...
        container.append('div').classed('sdd-component', true).attr('id', 'sdd-chart');
        container.append('div').classed('sdd-component', true).attr('id', 'sdd-listing');
    }
